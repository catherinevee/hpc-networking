...
inputs = {
  ...
  security_group_ids = [aws_security_group.fsx_scratch.id]
  ...
  data_repository_path = "s3://${dependency.s3_bucket.outputs.bucket_name}/scratch"
...
resource "aws_security_group" "fsx_scratch" {
  name_prefix = "hpc-${local.environment}-fsx-scratch-"
  vpc_id      = dependency.vpc.outputs.vpc_id
  description = "Security group for FSx Lustre scratch storage"
  ...
  tags = merge(local.common_tags, {
    Name = "hpc-${local.environment}-fsx-scratch-sg"
